<template>
  <div class="relative bg-white">
    <div class="overflow-y-auto">
      <DataTable v-model:filters="filters" class="p-datatable-sm" :value="adminStore.rates" filter-display="row"
        scrollable scroll-height="50vh">
        <template #footer>
          <div class="flex justify-between w-full">
            <Button label="Додати" title="Додати рейт" icon="pi pi-plus" icon-pos="right"
              @click="isAddingRate = !isAddingRate" />
            <Button label="Завантажити" title="Завантажити" icon="pi pi-save" severity="success" icon-pos="right"
              @click="updateRate" />
          </div>
        </template>

        <Column field="clientName" header="Клієнт" :show-filter-menu="false" filter-field="clientName">
          <template #filter="{}">
            <InputText v-model="filters['clientName'].value" placeholder="Пошук по клієнту" class="p-column-filter" />
          </template>
        </Column>
        <Column field="botURI" header="URI Бота" filter-field="botURI" :show-filter-menu="false">
          <template #filter="{}">
            <InputText v-model="filters['botURI'].value" placeholder="Пошук по боту" class="p-column-filter" />
          </template>
        </Column>
        <Column field="country" header="Країна" filter-field="country" :show-filter-menu="false">
          <template #filter="{}">
            <InputText v-model="filters['country'].value" placeholder="Пошук по країні" class="p-column-filter" />
          </template>
        </Column>
        <Column field="contract" header="Договір" />
        <Column field="rate" header="Рейт">
          <template #body="slotProps">
            <InputNumber v-model="slotProps.data.rate" :max-fraction-digits="2" input-id="currency-germany"
              mode="currency" currency="EUR" locale="de-DE" @input="initUpdatedRates(slotProps.data)" />
          </template>
        </Column>
        <Column>
          <template #body="slotProps">
      ...
      <div class="grid h-[60vh] w-full grid-cols-1 gap-4 lg:hidden lg:h-auto">
        <div v-for="item in adminStore.rates" :key="item.id" class="flex flex-col gap-3 p-4 bg-white rounded-lg shadow">

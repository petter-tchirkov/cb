  ...
  <section>
    ...
    <div class="flex flex-col items-center p-5">
      <div class="flex items-end justify-center w-full gap-10 mb-5">
        ...
        <ui-input v-model="filterParams.country" class="grow" label="Пошук по країні" type="text">
          <template #icon>
            <Icon name="ant-design:search-outlined" />
          </template>
        </ui-input>
        <div class="flex gap-5">
          <ui-datepicker v-model="filterParams.date_from" label="Дата" />
          <ui-datepicker v-model="filterParams.date_to" label="Дата" />
        </div>
        <ui-button label="Відфільтрувати" class="mt-[8px]" @click="filterData" />
      </div>
      <ui-table :items="costsStore.costs" :headers="headers">
        <ui-table-row v-for="(row, index) in costsStore.costs" :key="index">
      ...
      <div class="flex gap-3 mt-5">
        <ui-table-pagination v-if="!filterParams.show_all" :current-page="filterParams.page"
          :per-page="filterParams.per_page" :total-pages="costsStore.pages" @next-page="nextPage"
          @previous-page="previousPage" @update="(page) => goToPage(page)" />
        <ui-button :label="filterParams.show_all ? 'Показати пагінацію' : 'Показати все'" @click="showHideAllFields" />
      </div>
    </div>
  </section>
</template>
